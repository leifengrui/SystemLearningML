...
> [!note] 解答：怎么理解各 rank 持全量参数？92B 模型各卡显存放得下吗？这里的"参数"指什么？
>
> 三个子问题一次讲透，因为它们其实是 DDP 适用边界的核心。
>
> ### 1. "各 rank 持全量参数"是什么意思
>
> DDP 是**数据并行**，不是模型并行。它的设定是：**每张卡（每个 rank）都完整地、独立地持有一份完整的模型**——所有权重、所有 buffer。N 张卡就是 N 份完整的模型副本。各 rank 之间唯一不同的是**各自跑的数据 batch 不同**。
>
> ```
> rank 0:  model_0 = 完整模型(所有权重)  ← 跑 batch_0
> rank 1:  model_1 = 完整模型(所有权重)  ← 跑 batch_1   (与 model_0 权重初始相同)
> ...
> rank N-1: model_{N-1} = 完整模型        ← 跑 batch_{N-1}
> ```
>
> 注意：这 N 份模型权重**初始时相同**（rank 0 广播给其他 rank），训练过程中因为各 rank 用"全体平均梯度"更新，所以**每步更新后权重仍然相同**——这就是 DDP 的"参数同步"。DDP 不切模型，切的是数据。
>
> ### 2. 这里的"参数"指什么
>
> 就是模型的**所有可学习权重 + 不可学但要搬的 buffer**，具体到 LLM：
>
> | 组成 | 是否参与 backward | 单卡显存占用（fp32+AdamW） |
> |---|---|---|
> | **权重 parameters**（embedding、QKV 投影、FFN 的 gate/up/down、LM head 等） | ✅ | $4P$ 字节（$P$=参数数量） |
> | **梯度**（backward 算出来，与权重同 shape） | 临时 | $4P$ |
> | **优化器状态**（AdamW 的 $m,v$ 两个一阶/二阶矩） | 状态 | $8P$ |
> | **buffer**（RoPE 的 inv_freq、register_buffer 注册的） | ❌ 但要存 | 通常很小 |
>
> "92B 模型"指的是**权重参数量 $P = 9.2\times10^{10}$**（约 920 亿个权重元素）。注意这是"参数数量"，不是字节：
>
> - fp32（4 字节/参数）：$92\text{B} \times 4 = 368$ GB 仅权重；
> - bf16（2 字节/参数）：$92\text{B} \times 2 = 184$ GB 仅权重；
> - 训练态还要加梯度 + AdamW 状态：fp32 master 权重 + fp32 的 $m,v$ + bf16 梯度 ≈ 又加 $4P+8P+2P = 14P$，按 fp32 训练总 $\approx 16P$。
>
> ### 3. 92B 模型单卡能装下吗——这是 DDP 的生死线
>
> DDP 的**硬前提**：**单卡显存能装下"完整模型 + 梯度 + 优化器状态 + 激活"**。算一算 92B：
>
> | 项 | fp32 训练态 | bf16 混合精度训练态 |
> |---|---|---|
> | 权重 | 368 GB | 184 GB（bf16）+ 368 GB（fp32 master）= 552 GB |
> | 梯度 | 368 GB | 184 GB（bf16） |
> | AdamW $m,v$ | 736 GB | 736 GB（fp32） |
> | **小计（不含激活）** | **1472 GB** | **1472 GB**（混合精度下也接近） |
> | 单卡 A100/H100 显存 | 80 GB | 80 GB |
>
> **结论：92B 模型单卡 80GB 显存根本装不下训练态，连权重都装不下（184GB > 80GB）**。所以 DDP 在 92B 上**直接不可用**——你在每张卡上 `model = MyLLM().cuda()` 这一步就 OOM 了，根本到不了反向/同步那一步。
>
> 那 DDP 适合多大？经验上**单卡能装下完整训练态**的模型：A100 80GB 上约 **7B~13B 混合精度训练**勉强可行（13B bf16 权重 26GB + AdamW 状态 ~104GB... 其实也吃力，常需 CPU offload 或 ZeRO-1）。**70B 及以上单卡装不下，必须升级到 [[Fully Sharded Data Parallel|FSDP]] 或张量并行（[[Megatron-LM]] 的 TP）**——这正是 FSDP 存在的理由：把"参数/梯度/优化器状态分片到 N 卡"，让单卡显存从 $16P$ 降到 $16P/N$，70B 在 8 卡可训。
>
> ### 4. "各 rank 持全量参数"为什么不是浪费
>
> 看起来 N 张卡各存一份完整模型是"N 倍冗余"。但 DDP 的价值不在省显存，而在**提吞吐**：N 张卡同时跑 N 个不同 batch，等效 batch 放大 N 倍，训练吞吐近线性增长。显存冗余是它的代价，省显存是 FSDP/ZeRO/Megatron 的事。所以：
>
> - **模型单卡装得下 + 要提吞吐** → DDP；
> - **模型单卡装不下** → FSDP（数据并行维分片）/ Megatron TP（模型维切分）/ PP（层间切分）。
>
> ### 速记
>
> - **DDP = 每卡一份完整模型 + 切数据**，参数指全部权重+buffer，N 份是冗余但换吞吐；
> - **92B 单卡装不下**（bf16 权重就 184GB > 80GB），DDP 直接 OOM，必上 FSDP/Megatron；
> - DDP 的适用边界就是"单卡训练态显存"，越过这条线就是 FSDP/TP/PP 的领域，见 [[Fully Sharded Data Parallel]] §2、[[Megatron-LM]] §2。
## 2. 为什么需要它（动机与背景）
