...
- **下游（应用）**: 对齐后 LLM（InstructGPT/Llama-chat）、[[DPO]]/[[IPO]]/[[KTO]]（绕过 PPO 的替代）、[[reward hacking]]/[[KL explosion]]/[[policy collapse]]/[[mode collapse]]（PPO 失败模式）、[[RLAIF]]（用 AI 替人类标偏好）、[[GRPO]]（PPO 变体）、[[DAPO]]（GRPO+4 trick 的 long-CoT 特化版，开源复现 R1 风格 RL 的标杆）。
- **对比 / 易混**:
