...
> 
> > [!note] 解答：Adam 只有 2 个 state，为什么是权重的 4×？
> >
> > "4×"是**两个因子乘出来的**——2 个 state × fp32 vs fp16 权重 = 2 × 2 = 4。拆开看：
> >
> > ### 1. 字节账逐项算
> >
> > | 量 | 元素数 | 每元素字节 | 总字节 |
> > |---|---|---|---|
> > | 权重 $\theta$（fp16 训练） | $N$ | 2 | $2N$ |
> > | Adam state $m$ | $N$ | 4（fp32） | $4N$ |
> > | Adam state $v$ | $N$ | 4（fp32） | $4N$ |
> > | **state 合计** $m+v$ | $2N$ | 4 | **$8N$** |
> >
> > 比值 $\dfrac{M_{\text{state}}}{M_W}=\dfrac{8N}{2N}=4$。
> >
> > ### 2. 两个因子各占几倍
> >
> > - **因子 A：2 个 state** → Adam 存了 $m$ 和 $v$ 两份，每份都与参数同形状，所以"份数"上是权重的 **2×**。
> > - **因子 B：fp32 vs fp16** → 每个 state 元素 4 字节（fp32），权重元素 2 字节（fp16），所以"每元素字节"上是权重的 **2×**。
> >
> > $2 \times 2 = 4$，这就是 "Adam state 是 fp16 权重的 4×" 的来源。
> >
> > ### 3. 反例验证：换精度就不是 4× 了
> >
> > | 权重精度 | state 精度 | 比值 | 推论 |
> > |---|---|---|---|
> > | **fp16 (2B)** | **fp32 (4B) × 2 state** | **8/2 = 4×** ← 本笔记场景 | 7B → 权重 14GB，state 56GB |
> > | fp32 (4B) | fp32 (4B) × 2 state | 8/4 = 2× | 全 fp32 训练时 state 只比权重大 2× |
> > | fp16 (2B) | fp32 (4B) × 1 state（如 SGD+momentum） | 4/2 = 2× | 只 1 个 state，2× 权重 |
> > | fp16 (2B) | int8 (1B) × 2 state（8-bit Adam） | 2/2 = 1× | 8-bit Adam 的 state 与 fp16 权重同大 |
> > | bf16 (2B) | fp32 (4B) × 2 state | 8/2 = 4× | bf16 权重仍 2B，故 4× 仍成立 |
> >
> > 看最后两行就明白——8-bit Adam 之所以"减 4×"（56→14GB），就是把因子 B 从 4B 降到 1B（× 1/4），4× 变 1×。同理 SGD+momentum 之所以"减 2×"（56→28GB），是把因子 A 从 2 降到 1（少一个 $v$），4× 变 2×。
> >
> > ### 4. 为什么是 fp32 不跟着权重一起 fp16
> >
> > 看 §2.3 已展开：$v$ 存梯度平方，fp16 会下溢（$g^2$ 极小）；$m$ 要跨 step 累加，fp16 累加误差大。所以**权重可以 fp16 省显存，state 必须保 fp32 保数值稳定**——这正是"4×"里因子 B=2× 的根因，也是 [[mixed precision training]] 的核心设计：算的部分（权重/梯度）降精度，记的部分（state/master 权重）保 fp32。详见 [[mixed precision training]]、[[loss scaling]]、[[数值类型与精度]]。
> >
> > ### 5. 一句话速记
> >
> > > 4× = 2 个 state × fp32/fp16 = 2 倍 × 2 倍。换 8-bit Adam（state 1B）就变 1×，换 SGD+momentum（1 个 state）就变 2×，换全 fp32 训练（权重 4B）也变 2×——四个数对得上就懂了。
