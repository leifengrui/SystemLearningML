...
> [!note] 解答："方差"在这里指梯度估计量 $\hat g$ 的随机波动大小——它决定 PG 能不能训得动
> 先把"方差"二字在 PG 语境里精确化。它**不是**指 reward 的方差、也不是指参数 $\theta$ 的方差，而是指**梯度估计量 $\hat g$ 作为一个随机变量的方差** $\text{Var}[\hat g]$。下面分层讲透。
>
> ### 1. 方差到底是什么的方差
> 策略梯度是**蒙特卡洛估计**：采 N 条轨迹，每条算一个梯度样本 $X_i=\sum_t\nabla_\theta\log\pi_\theta(a_t\mid s_t)\,c_t$（$c_t$ 是 $G_t$ 或 $A_t$ 等权重），再平均：
> $$\hat g=\frac1N\sum_{i=1}^N X_i,\qquad \text{Var}[\hat g]=\frac{\text{Var}[X]}{N}$$
> 这里 $X_i$ 因轨迹不同而不同（环境随机 + 策略采样随机），是个**随机变量**。$\text{Var}[X]$ 大 → $\hat g$ 在不同 batch 之间抖得厉害 → "这一次算出的梯度"和"真实梯度 $\nabla J$"偏差大。**方差 = 梯度估计的噪声水平**。
>
> ### 2. 为什么方差是 PG 的头号敌人（直觉版）
> 想象蒙眼下山：梯度 = 脚探出的"哪边是下坡"。若估计准（低方差），每步可靠迈；若估计抖（高方差），探出来的方向时左时右，只能迈极小的步子（小学习率）否则一脚踩空发散。后果链：
> $$\text{Var 大}\Rightarrow\hat g\text{ 噪声大}\Rightarrow\text{学习率必须极小}\Rightarrow\text{收敛慢/震荡/易陷局部最优}$$
> 实测：裸 REINFORCE 学习率 $\eta>10^{-3}$ 就发散，$\eta\le10^{-4}$ 才稳但慢到不可用，不同 seed 结果天差地别——都是方差害的。
>
> ### 3. 方差的来源（为什么 PG 天然大方差）
> 1. **轨迹间方差**：同一策略跑不同轨迹，return $G_t$ 动辄差几个数量级（赢 +100 vs 输 -10），作权重时梯度被这种巨差放大；
> 2. **时间间方差**：一条轨迹内不同 $t$ 的 $G_t$ 量级差（早期 reward 累积长、末期短）；
> 3. **状态相关方差**：状态本身好坏（好状态不管怎么走都赢、坏状态反之），这部分与"动作好坏"无关却进了梯度 → baseline 专减它；
> 4. **动作采样方差**：$\pi$ 采到不同 $a_t$；
> 5. **小批量方差**：batch 太小估不准。
>
> ### 4. 数学上"降方差"到底降的是什么
> 看 §4.2 的公式：$\text{Var}[\nabla\log\pi\,(G-b)]=\mathbb{E}[(\nabla\log\pi)^2(G-b)^2]-\mathbb{E}[\nabla\log\pi(G-b)]^2$。
> - **第二项**（期望平方）= 真实梯度的平方，**baseline 不改它**（无偏证明保证）；
> - **第一项**（二阶矩）= 梯度样本的平均"能量"，baseline 让 $(G-b)^2$ 变小 → 二阶矩降 → 方差降。
> 当 $b=V^\pi(s)$ 时 $G-V$ 是"残差"（只留动作贡献、扣掉状态底色），二阶矩最小，方差下降量 $\approx\text{Var}[V]$（状态底色部分被扣掉）。**所以"降方差"= 把梯度样本的能量/波动压下去，但保留期望（方向）不变**。
>
> ### 5. 方差 vs 偏差的 trade-off（为什么能"用偏差换方差"）
> - **MC（$G_t$）**：无偏但高方差；
> - **TD（用 $V_\phi$）**：有偏（$V_\phi$ 估不准时）但低方差；
> - **GAE（$\lambda$）**：在二者间连续插值，$\lambda\to0$ 偏差大/方差小，$\lambda\to1$ 偏差小/方差大。
> PG 实践常接受小偏差换大方差下降，因为**方差才是收敛瓶颈**——梯度方向偏一点但稳，比方向准但抖到训不动强得多。
>
> ### 6. 一句话总结
> **方差 = 梯度估计量 $\hat g$ 在不同采样下的随机波动；PG 是蒙特卡洛估计天然大方差；方差大→学习率必须小→训不动；variance reduction 的所有技巧（baseline/advantage/GAE/归一化/clipping/多 env/entropy）都在压 $\text{Var}[\hat g]$，让学习率能放大、训练能收敛。** 详见本文 §3.2 技术清单、§4 数学原理。
>
> 相关：[[baseline]] §4.2 也从公式侧讲同一件事；本条侧重"方差"二字的语义澄清。
